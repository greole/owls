...
class Bokeh:
    ...
    def draw(
        ...
        **kwargs
        ...
        if func == "quad":
            getattr(figure, func)(top=y, bottom=0, left=x[:-1], right=x[1:], **kwargs)
            return figure
...
rcParams = {
    "fig_size": {"min_border": 12, "plot_height": 350, "plot_width": 300},
    "font": {
...
class Snappy:
    ...
    def display(self, size, media_type="mp4"):
        if media_type == "mp4":
            i = (
                '<video width={} height="222" controls="controls"> <source src={}'
                ' type="video/mp4" /></video>'
            )
        else:
            i = (
                "<img style='width: {}px; margin: 0px; float: left; border: 1px solid"
                " black;' src='{}' />"
            )
