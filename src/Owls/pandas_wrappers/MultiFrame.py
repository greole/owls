...
class MultiFrame:
    ...
    def from_dict(input_dict, **kwargs):
        return MultiFrame(
            cases=[
                FoamFrame.from_dict(d, name=name, **kwargs)
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, NumberTypes):
            return MultiFrame(
                OrderedDict([(i, c.__mul__(other)) for i, c in self.cases.items()])
            )
        else:  # MultiFrame
            return MultiFrame(
                OrderedDict(
                    [(i, c.__mul__(other.cases[i])) for i, c in self.cases.items()]
                )
            )
    ...
    def __rmul__(self, other):
        ...
        if isinstance(other, NumberTypes):
            return MultiFrame(
                OrderedDict([(i, c.__rmul__(other)) for i, c in self.cases.items()])
            )
        else:  # MultiFrame
            return MultiFrame(
                OrderedDict(
                    [(i, c.__rmul__(other.cases[i])) for i, c in self.cases.items()]
                )
            )
    ...
    def __truediv__(self, other):
        ...
        if isinstance(other, NumberTypes):
            return MultiFrame(
                OrderedDict([(i, c.__truediv__(other)) for i, c in self.cases.items()])
            )
        else:  # MultiFrame
            return MultiFrame(
                OrderedDict(
                    [(i, c.__rtruediv__(other.cases[i])) for i, c in self.cases.items()]
                )
            )
    ...
    def __rtruediv__(self, other):
        ...
        if isinstance(other, NumberTypes):
            return MultiFrame(
                OrderedDict([(i, c.__rtruediv__(other)) for i, c in self.cases.items()])
            )
        else:  # MultiFrame
            return MultiFrame(
                OrderedDict(
    ...
    def __add__(self, other):
        """add self with other"""
        return MultiFrame(
            OrderedDict([(i, c.__add__(other.cases[i])) for i, c in self.cases.items()])
        )
    ...
    def __radd__(self, other):
        ...
        return MultiFrame(
            OrderedDict(
                [(i, c.__radd__(other.cases[i])) for i, c in self.cases.items()]
            )
        )
    ...
    def __sub__(self, other):
        """add self with other"""
        return MultiFrame(
            OrderedDict([(i, c.__sub__(other.cases[i])) for i, c in self.cases.items()])
        )
    ...
    def __rsub__(self, other):
        ...
        return MultiFrame(
            OrderedDict(
                [(i, c.__rsub__(other.cases[i])) for i, c in self.cases.items()]
            )
        )
            ...
            for l in baseCase.locations:
                ...
                for (name, case), param in zip(self.cases.items(), params):
                    ...
                    try:
                        ...
                        deltaF = Series(
                            case.location(l)[fieldName].values - baseField.values
                        )
                        print(deltaF)
    ...
    def time_average(self, suffix="Avg", time_start=0.0):
        ...
        return MultiFrame(
            [
                case.time_average(suffix, time_start)
    ...
    def on(self, case, func, **kwargs):
        ...
        return MultiFrame(
            [(case_ if cname != case else n) for cname, case_ in self.cases.items()]
        )
